...
tf.app.flags.DEFINE_boolean('print_verbose', False,
...
def create_MaskGAN(hparams, is_training):
    ...
    new_learning_rate = tf.placeholder(
        tf.float32, [], name='new_learning_rate')
    ...
    with tf.name_scope('discriminator_losses'):
        ...
        tf.summary.scalar('dis_loss_prob_fake_sequence',
                          tf.exp(-dis_loss_fake))
        tf.summary.scalar('dis_loss_real_sequence', dis_loss_real)
        tf.summary.scalar('dis_loss_prob_real_sequence',
                          tf.exp(-dis_loss_real))
        ...
        with tf.device(tf.train.replica_device_setter(FLAGS.ps_tasks)):
            ...
            with tf.container(container_name):
                ...
                if FLAGS.num_rollouts == 1:
                    ...
                    model = rollout.create_rollout_MaskGAN(
                        hparams, is_training)
                else:
                    raise ValueError
                ...
                with sv.managed_session(FLAGS.master) as sess:
                    ...
                    while not sv.ShouldStop():
                        ...
                        for i in range(dis_offset):
                            ...
                            if FLAGS.data_set == 'ptb':
                                # Statefulness of the Generator being used for
                                # Discriminator.
                        ...
                        for x, y, _ in iterator:
                            for _ in xrange(hparams.dis_train_iterations):
                                ...
                                if FLAGS.data_set == 'ptb':
                                    for i, (c, h) in enumerate(model.fake_gen_initial_state):
                                        train_feed[
                                            c] = dis_initial_state_eval[i].c
                                        train_feed[
                                            h] = dis_initial_state_eval[i].h

                                _, dis_loss_eval, step = sess.run(
                                    [model.dis_train_op, model.dis_loss,
                                        model.global_step],
                            ...
                            # Construct the train feed.
                            train_feed = {model.inputs: x,
                                          model.targets: y, model.present: p}
                            ...
                            if FLAGS.data_set == 'ptb':
                                ...
                                for i, (c, h) in enumerate(model.fake_gen_initial_state):
                                    train_feed[
                                        c] = fake_gen_initial_state_eval[i].c
                                    train_feed[
                                        h] = fake_gen_initial_state_eval[i].h
                            ...
                            # Determine the state had the Generator run over
                            # real data.
                            [gen_initial_state_eval, fake_gen_initial_state_eval] = sess.run(
                            ...
                            if is_chief and (step / FLAGS.summaries_every >
                                ...
                                geometric_avg = compute_geometric_average(
                                    avg_percent_captured)
                            ...
                            if is_chief and (step / FLAGS.print_every > print_step_division):
                                print_step_division = (
                                    step / FLAGS.print_every)
                                print('global_step: %d' % step)
                                print(' perplexity: %.3f' % perplexity)
                                print(' gen_learning_rate: %.6f' %
                                      gen_learning_rate)
                                log.write('global_step: %d\n' % step)
                                log.write(' perplexity: %.3f\n' % perplexity)
                                log.write(' gen_learning_rate: %.6f' %
                                          gen_learning_rate)

                                # Average percent captured for each of the
                                # n-grams.
                                avg_percent_captured = {
                                    '2': 0., '3': 0., '4': 0.}
                                for n, data_ngram_count in data_ngram_counts.items():
                                ...
                                log.write(' geometric_avg: %.3f.' %
                                          geometric_avg)
                                arithmetic_avg = compute_arithmetic_average(
                                    avg_percent_captured)
                                print(' arithmetic_avg: %.3f.' %
                                      arithmetic_avg)
                                log.write(' arithmetic_avg: %.3f.' %
                                          arithmetic_avg)
