...
tf.app.flags.DEFINE_string('output_file', 'reviews.txt',
                            'Sample output file.')
        ...
        with sv.managed_session(
            ...
            for n in xrange(FLAGS.number_epochs):
                ...
                for x, y, _ in iterator:
                    ...
                    generate_logs(sess, model, output_file,
                                  id_to_word, eval_feed)
            output_file.close()
...
def main(_):
    ...
    output_file = tf.gfile.GFile(
        os.path.join(FLAGS.output_path, FLAGS.output_file), mode='w')
